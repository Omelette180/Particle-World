...
ElementRegistry.register({id:'shrapnel',name:'Shrapnel',category:'explosives',color:'#888888',density:3.0,state:'solid',desc:'Metal fragments from explosions. Falls fast.',heatConductivity:0.3,tick(x,y,g,h){for(let i=0;i<3;i++){if(!g.isEmpty(x,y+1)){const t=g.getType(x,y+1);if(t&&t!=='wall'){const nc=g.getCell(x,y+1);if(nc)nc.heat=(nc.heat??0)+100;g.clearCell(x,y);return;}break;}g.swapCells(x,y,x,y+1);}});
